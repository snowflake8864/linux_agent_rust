...
const DB_PATH: &str = concat!("/opt/osec/db", "/jump.db");
...
pub fn upsert(row: &JumpStatusRow) -> Result<()> {
    ...
    conn.execute(
        ...
        params![
            row.current_ip, row.source_ip, row.target_ip, row.gateway, row.mode,
            row.current_password, row.last_ip_jump_time, row.last_pw_jump_time,
            row.last_pw_jump_user,
            row.ip_scheme, row.ip_cycle_label, row.ip_timing_label, row.ip_way_label,
            row.pw_scheme, row.pw_cycle_label, row.pw_timing_label, row.updated_at,
        ],
    )?;
    Ok(())
}
