...
pub fn init_table() -> Result<()> {
    ...
    let _ = conn.execute_batch(
        "ALTER TABLE alert_log ADD COLUMN identifier TEXT NOT NULL DEFAULT '';",
    );
    let _ = conn.execute_batch(
        "ALTER TABLE alert_log ADD COLUMN n_type INTEGER NOT NULL DEFAULT 0;",
    );
    // 时间字段从 TEXT 迁移为 INTEGER (Unix 时间戳秒)
    let _ = conn.execute_batch(
        "ALTER TABLE alert_log ADD COLUMN handled_at_ts INTEGER NOT NULL DEFAULT 0;",
    );
    let _ = conn.execute_batch(
        "ALTER TABLE alert_log ADD COLUMN created_at_ts INTEGER NOT NULL DEFAULT 0;",
    );
    Ok(())
}
...
pub fn insert(row: &AlertLogRow, max_rows: u32) -> Result<i64> {
    ...
    conn.execute(
        ...
        params![
            ...
            row.n_type,
            row.handle_status,
            row.handle_status_label,
            row.handle_user,
            row.handled_at.to_string(),
            row.handled_at,
...
pub fn query_page(
    ...
    let collected: Result<Vec<_>> = stmt
        .query_map(
