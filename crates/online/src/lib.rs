...
impl BaseOnline {
    ...
    pub async fn run(net_client: &mut NetClient) -> Result<String, String> {
        ...
        println!("==url:{}", url);
        match net_client.post_data_async(&url, &json_str, Duration::from_secs(30), None, Some("auth.json"),).await {
            Ok(response) => {
...
impl StartOnline for BootManager {
    fn start_services(&mut self, token_tx: mpsc::Sender<String>, mut host_is_offline_rx: mpsc::Receiver<bool>) -> Pin<Box<dyn Future<Output = Result<String, String>> + Send + '_>> {
        Box::pin(async move {
            ...
            let app_path = NETINFO_CONFIG.lock().unwrap().app_path.clone();

            loop {
                let base_url = self.get_base_url();
                println!("0====================================================="); 
                let mut net_client = match NetClient::new(base_url, true, is_offline, Some(app_path.clone())) {
                    Ok(client) => client,
                    Err(err) => {
                ...
                match BaseOnline::run(&mut net_client).await {
                    Ok(token) => {
                        ...
                        log_info!("Token 已成功发送 token:{}！",token);
                                ...
                                _ = interval.tick() => {
                                    if let Some(json_data) = get_system_metrics() {
                                        ...
                                        match net_client.post_data_write_async(
                                            ...
                                            Some(&token),
                                            Some("puthardwareinfo.json"),
                                                    Some(WriteMode::Overwrite),
                                        ).await {
                                            Ok(response) => {/*log_info!("response:{}",response);*/},
