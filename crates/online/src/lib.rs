...
impl BaseOnline {
    pub fn new() -> Self {
        let hostInfo = HostInfo::from_ini();
        BaseOnline {
            uid: hostInfo.uid,
            macid: hostInfo.macid,
            ip: hostInfo.ip,
            ver: hostInfo.ver,
            type_: 1,
            os: hostInfo.os,
            memsize: hostInfo.memsize,
            cpu: hostInfo.cpu,
            hdsize: hostInfo.hdsize,
    ...
    pub async fn run(net_client: &mut NetClient) -> Result<String, String> {
        let mut base_online = BaseOnline::new();
...
impl StartOnline for BootManager {
    fn start_services(&mut self, token_tx: mpsc::Sender<String>, host_is_offline_rx: mpsc::Receiver<bool>) -> Pin<Box<dyn Future<Output = Result<String, String>> + Send + '_>> {
        Box::pin(async move {
            let mut host_is_offline_rx = host_is_offline_rx;
            ...
            loop {
                ...
                match BaseOnline::run(&mut net_client).await {
                    Ok(token) => {
                        if let Err(err) = token_tx.send(token).await {
                            eprintln!("发送 token 失败: {}", err);
                            continue; // 如果发送失败，重新获取 token
                        }
                        println!("Token 已成功发送！");
                    }
                    Err(err) => {
                        eprintln!("获取 token 时发生错误: {}", err);
                        continue; // 如果获取 token 失败，重新尝试
                    }
                }

                // 第二步：监听 host_is_offline_rx 信号
                println!("开始监听 host_is_offline 信号...");
                match host_is_offline_rx.recv().await {
                    Some(true) => {
                        println!("收到 host_is_offline 为 true 的信号，重新获取 token...");
                        continue; // 返回到循环的开头，重新获取 token
                    }
                    Some(false) => {
                        println!("收到 host_is_offline 为 false 的信号，继续监听...");
                    }
                    None => {
                        eprintln!("host_is_offline_rx 信号通道已关闭，退出任务。");
                        break;
                    }
                }
            }
