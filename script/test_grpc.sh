...
#   ./test_grpc.sh <编号>        # 直接测试指定接口 (1-35, s1-s9, vf, vr)
#   ./test_grpc.sh all           # 测试全部只读接口 (1-34)
#   ./test_grpc.sh write         # 测试写接口（需离线模式）
#   ./test_grpc.sh stream        # 测试流式接口 (17, 18, s1-s6, vf, vr)
...
show_test_help() {
    ...
    case "$tid" in
        ...
        30)    echo -e "${CYAN}[30] PeripheralDefenseMode${NC} — 获取外设防护模式（读）"
               echo "    返回: mode (0=OFF, 1=MONITOR, 2=PROTECT)"
               echo "    用法: $0 30" ;;
        31)    echo -e "${CYAN}[31] PolicyQuery${NC} — 一次性查询全部策略（与 /opt/osec/policies/ 一致）"
        ...
        s3)    echo -e "${CYAN}[s3] 停止扫描${NC} — 停止指定 scan_id 的扫描任务"
               echo "    可用环境变量 SCAN_ID 指定 scan_id，默认 test-scan"
               echo "    用法: SCAN_ID=<scan_id> $0 s3" ;;
        s4)    echo -e "${CYAN}[s4] 暂停扫描${NC} — 暂停指定 scan_id 的扫描任务"
               echo "    可用环境变量 SCAN_ID 指定 scan_id，默认 test-scan"
               echo "    用法: SCAN_ID=<scan_id> $0 s4" ;;
        s5)    echo -e "${CYAN}[s5] 恢复扫描${NC} — 恢复指定 scan_id 的扫描任务"
               echo "    可用环境变量 SCAN_ID 指定 scan_id，默认 test-scan"
               echo "    用法: SCAN_ID=<scan_id> $0 s5" ;;
        ...
        s9)    echo -e "${CYAN}[s9] 隔离区列表(按病毒名)${NC} — ListQuarantine filter=\"virus:xxx\""
               echo "    用法: $0 s9" ;;
        vf)    echo -e "${CYAN}[vf] 病毒扫描完整流程${NC} — 启动→暂停→恢复→停止（自动使用真实 scan_id 串联）"
               echo "    扫描 /usr，验证 scan_id 获取、pause/resume/stop 均 success=true"
               echo "    用法: $0 vf" ;;
        vr)    echo -e "${CYAN}[vr] 病毒扫描递归完整性${NC} — 多级子目录文件都应被扫描"
               echo "    在 /tmp 创建含多级子目录的测试目录，验证 total_scanned 覆盖子目录文件"
               echo "    用法: $0 vr" ;;
        ...
        *)     echo -e "${RED}未知测试编号: $tid${NC}"
                echo "有效范围: 1-33, s1-s9, vf, vr, w1-w17"
...
test_31() { grpc_call "全部策略(一次查所有,与/opt/osec/policies/一致)" \
...
test_s3() { virus_scan_stream_test "病毒扫描-停止扫描" \
    "{\"stop_scan\":{\"scan_id\":\"${SCAN_ID:-test-scan}\"}}" '"stop_response"'; }
test_s4() { virus_scan_stream_test "病毒扫描-暂停扫描" \
    "{\"pause_scan\":{\"scan_id\":\"${SCAN_ID:-test-scan}\"}}" '"pause_response"'; }
test_s5() { virus_scan_stream_test "病毒扫描-恢复扫描" \
    "{\"resume_scan\":{\"scan_id\":\"${SCAN_ID:-test-scan}\"}}" '"resume_response"'; }
test_s6() { virus_scan_stream_test "病毒扫描-处置文件" \
    '{"dispose_file":{"scan_id":"","file_path":"/tmp/test.txt","action":1}}' '"dispose_result"'; }

# 发送单个 VirusScan ClientMessage，返回原始输出（不判定 PASS/FAIL）
virus_scan_send() {
    local json="$1" duration="${2:-6}"
    printf '%s\n' "$json" | timeout "$duration" grpcurl -plaintext -emit-defaults \
        -import-path "$PROTO_DIR" \
        -proto common.proto -proto virus_scan.proto \
        -d @ \
        -connect-timeout 3 \
        "$GRPC_ADDR" virus_scan.VirusScanService/StreamControl 2>&1
}

# 从 start_scan 响应中提取 scan_id（兼容 snake_case / camelCase）
extract_scan_id() {
    grep -oE '"(scan_id|scanId)"[[:space:]]*:[[:space:]]*"[^"]+"' \
        | head -1 \
        | sed -E 's/.*:[[:space:]]*"([^"]+)"[[:space:]]*$/\1/'
}

# vf: 病毒扫描完整流程（启动→暂停→恢复→停止，使用真实 scan_id 串联）
test_virus_full_flow() {
    echo -e "${CYAN}[TEST]${NC} 病毒扫描完整流程（启动→暂停→恢复→停止）"
    local start_out scan_id out

    # 1. 启动扫描（/usr 足够大，扫描会持续运行，保证后续 pause/stop 命中运行态）
    start_out=$(virus_scan_send '{"start_scan":{"target":"/usr","include_script":false,"full_disk":false}}' 6)
    scan_id=$(echo "$start_out" | extract_scan_id)
    if [ -z "$scan_id" ] || ! echo "$start_out" | grep -q '"start_response"'; then
        echo -e "  ${RED}[1/4] 启动扫描 FAIL${NC}（未获取到 scan_id）"
        echo "$start_out" | sed 's/^/    /'
        ((fail++))
        return
    fi
    echo -e "  ${GREEN}[1/4] 启动扫描 PASS${NC} scan_id=$scan_id"
    ((pass++))

    # 2. 暂停
    out=$(virus_scan_send "{\"pause_scan\":{\"scan_id\":\"$scan_id\"}}" 4)
    if echo "$out" | grep -q '"pause_response"' && echo "$out" | grep -qE '"success"[[:space:]]*:[[:space:]]*true'; then
        echo -e "  ${GREEN}[2/4] 暂停扫描 PASS${NC}"
        ((pass++))
    else
        echo -e "  ${RED}[2/4] 暂停扫描 FAIL${NC}（可能扫描已完成或状态不匹配）"
        echo "$out" | sed 's/^/    /'
        ((fail++))
    fi

    # 3. 恢复
    out=$(virus_scan_send "{\"resume_scan\":{\"scan_id\":\"$scan_id\"}}" 4)
    if echo "$out" | grep -q '"resume_response"' && echo "$out" | grep -qE '"success"[[:space:]]*:[[:space:]]*true'; then
        echo -e "  ${GREEN}[3/4] 恢复扫描 PASS${NC}"
        ((pass++))
    else
        echo -e "  ${RED}[3/4] 恢复扫描 FAIL${NC}"
        echo "$out" | sed 's/^/    /'
        ((fail++))
    fi

    # 4. 停止
    out=$(virus_scan_send "{\"stop_scan\":{\"scan_id\":\"$scan_id\"}}" 4)
    if echo "$out" | grep -q '"stop_response"' && echo "$out" | grep -qE '"success"[[:space:]]*:[[:space:]]*true'; then
        echo -e "  ${GREEN}[4/4] 停止扫描 PASS${NC}"
        ((pass++))
    else
        echo -e "  ${RED}[4/4] 停止扫描 FAIL${NC}"
        echo "$out" | sed 's/^/    /'
        ((fail++))
    fi
}

# vr: 病毒扫描递归完整性验证（多级子目录文件都应被扫描到）
test_virus_recursion() {
    echo -e "${CYAN}[TEST]${NC} 病毒扫描递归完整性（多级子目录文件都应被扫描）"
    local test_dir="/tmp/virus_scan_recursion_$$"
    mkdir -p "$test_dir/sub1" "$test_dir/sub2/sub3"
    printf 'x' > "$test_dir/root1.txt"
    printf 'x' > "$test_dir/root2.txt"
    printf 'x' > "$test_dir/sub1/a.txt"
    printf 'x' > "$test_dir/sub2/b.txt"
    printf 'x' > "$test_dir/sub2/sub3/c.txt"

    local out completed_scanned
    out=$(virus_scan_send "{\"start_scan\":{\"target\":\"$test_dir\",\"include_script\":false,\"full_disk\":false}}" 60)
    rm -rf "$test_dir"

    if ! echo "$out" | grep -q '"completed"'; then
        echo -e "  ${RED}递归扫描 FAIL${NC}（未收到 ScanCompleted）"
        echo "$out" | sed 's/^/    /'
        ((fail++))
        return
    fi

    completed_scanned=$(echo "$out" | grep -oE '"(total_scanned|totalScanned)"[[:space:]]*:[[:space:]]*[0-9]+' | head -1 | grep -oE '[0-9]+')
    if [ -n "$completed_scanned" ] && [ "$completed_scanned" -ge 5 ]; then
        echo -e "  ${GREEN}递归扫描 PASS${NC} total_scanned=$completed_scanned（>=5，覆盖多级子目录）"
        ((pass++))
    else
        echo -e "  ${RED}递归扫描 FAIL${NC} total_scanned=${completed_scanned:-未捕获}，预期 >= 5"
        echo "$out" | sed 's/^/    /'
        ((fail++))
    fi
}
...
show_menu() {
    ...
    echo -e "${CYAN}║${NC}  vf) 病毒扫描完整流程    vr) 病毒扫描递归完整性              ${CYAN}║${NC}"
    echo -e "${CYAN}╠══════════════════════════════════════════════════════╣${NC}"
    ...
    echo -e "${CYAN}║${NC}  ${YELLOW}stream${NC} 测试全部流式接口 (17, 18, s1-s6, vf, vr)        ${CYAN}║${NC}"
...
run_all_stream() {
    test_17; test_18
    test_s1; test_s2
    test_virus_full_flow   # vf: 启动→暂停→恢复→停止（真实 scan_id 串联）
    test_virus_recursion   # vr: 多级子目录递归完整性
    test_s6
}
...
case "${1:-menu}" in
    menu|"")
        ...
        while true; do
            ...
            case "$choice" in
                ...
                vf) test_virus_full_flow ;;
                vr) test_virus_recursion ;;
                ...
                \?|h|help)
                    ...
                    echo "  vf) 病毒扫描完整流程(启动→暂停→恢复→停止)  vr) 病毒扫描递归完整性"
                    echo ""
                    ...
                    echo "  stream 测试全部流式 (17, 18, s1-s6, vf, vr)"
                    echo "  full   测试全部 (读写+流)"
                    echo "  listen [秒]  监听告警流"
                    echo "  ?|h    显示此帮助    q    退出"
                    echo "  stat   快速查看在线状态"
    ...
    ?|help|-h|--help)
        ...
        echo "  $0 vf            病毒扫描完整流程（启动→暂停→恢复→停止）"
        echo "  $0 vr            病毒扫描递归完整性（多级子目录）"
        echo "  $0 w8            直接测试 CreateBackup"
        echo "  $0 all           测试全部只读接口 (1-33)"
        echo "  $0 write         测试全部写接口 (w1-w17)"
        echo "  $0 stream        测试流式接口 (17, 18, s1-s6, vf, vr)"
    ...
    vf) test_virus_full_flow; print_result ;;
    vr) test_virus_recursion; print_result ;;
    32) if [ -n "$2" ]; then
    ...
    *)
        ...
        if [[ "$arg" =~ ^w[1-9]$|^w1[0-7]$ ]]; then
            ...
            echo "无效参数: $1"
            echo "用法: $0 [?|help|all|write|stream|full|listen|<1-33>|s1-s9|vf|vr|w1-w17|menu]"
            echo "试试: $0 ?  查看完整帮助"
            exit 1
        fi
        print_result
